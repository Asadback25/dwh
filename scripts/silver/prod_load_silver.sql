...
BEGIN
    ...
    SELECT cst_id,
           ...
           cst_create_date
    FROM (SELECT *,
                 ROW_NUMBER() OVER (PARTITION BY cst_id ORDER BY cst_id DESC ) AS flag
    ...
    SELECT sls_ord_num,
           ...
           sls_cust_id,
           CASE
               WHEN sls_order_dt IS NULL OR LEN(sls_order_dt) != 8
                   THEN NULL
               ELSE CAST(CAST(sls_order_dt AS VARCHAR) AS DATE)
               END    sls_order_dt,
           CASE
               WHEN sls_ship_dt IS NULL OR LEN(sls_ship_dt) != 8
                   THEN NULL
               ELSE CAST(CAST(sls_ship_dt AS VARCHAR) AS DATE)
               END    sls_ship_dt,
           CASE
               WHEN sls_due_dt IS NULL OR LEN(sls_due_dt) != 8
                   THEN NULL
